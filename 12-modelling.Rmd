...
# The Area-Based Approach (ABA) to forest modelling {#modelling}

Here we provide an example demonstration on how to process ALS point cloud data to create wall-to-wall predictions of selected forest stand attributes using an area-based approach (ABA) to forest inventories. The steps used in this vignette as well as further details about enhanced forest inventories (EFI) are described in depth in [White et al. 2013](https://cfs.nrcan.gc.ca/publications?id=34887) and [White et al. 2017](https://cfs.nrcan.gc.ca/publications?id=38945). 

This vignette assumes that the user has a directory of classified and normalized `.las/.laz` files.  First we load the necessary packages
...
The function `readLAScatalog()` builds a `LAScatalog` object from a folder. Make sure the ALS files have associated index files - for details see: [Speed-up the computations on a LAScatalog](https://cran.r-project.org/web/packages/lidR/vignettes/lidR-computation-speed-LAScatalog.html).

```r
ctg <- readLAScatalog("F:\\petawawa\\ALS_Petawawa\\ALS2012\\ALS2012_norm_nobuff_filtered")
print(ctg)
#> class       : LAScatalog
...
To visualize the file pattern use the `plot()` function on the `LAScatalog` object.
