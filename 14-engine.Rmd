...
## Rationale for `LAScatalog` functionality {#engine-rationale}
...
Let's compute a DTM (see section \@ref(dtm)) as an example, where we use a `for` loop on 4 contiguous files. We can see the obvious invalidity of the DTM between the files, but also at the periphery, even if less obvious.
...
## The `LAScatalog` engine {#engine-engine}
...
## Read a collection of files {#engine-lascatalog}
...
In this and the following section (section \@ref(engine2)) we will use a collection of nine 500 x 500 m files to create examples. The nine files can be downloaded here:

1. [tiles_338000_5238000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338000_5238000_1.laz)
2. [tiles_338000_5238500.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338000_5238500_1.laz)
3. [tiles_338000_5239000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338000_5239000_1.laz)
4. [tiles_338500_5238000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338500_5238000_1.laz)
5. [tiles_338500_5238500.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338500_5238500_1.laz)
6. [tiles_338500_5239000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_338500_5239000_1.laz)
7. [tiles_339000_5238000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_339000_5238000_1.laz)
8. [tiles_339000_5238500.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_339000_5238500_1.laz)
9. [tiles_339000_5239000.laz](https://github.com/Jean-Romain/lidRbook/raw/master/data/ENGINE/catalog/tiles_339000_5239000_1.laz)
...
## Validation {#engine-asprs-compliance}

Similar to section \@ref(io), an important first step in ALS data processing is ensuring that your data is complete and valid. The `las_check()` function performs an inspection of `LAScatalog` objects for file consistency.
...
## Extract Regions of interest {#engine-clip}

### Extract a single ROI {#engine-clip-single}
...
### Multiple extractions {#engine-clip-multiple}
...
### Multiple extractions on disk {#engine-options-ondisk}
...
### Multiple extraction with point cloud indexation {#engine-options-index}
...
### Modifying buffers {#engine-options-chunk-buffer}
...
### Modify the chunk size {#engine-options-chunk-size}
...
### Parallel processing {#engine-options-parallel}
...
## Digital Terrain Model {#engine-dtm}

So far we have learned enough about the engine to generate a nice and valid DTM using `rasterize_terrain()` (see section \@ref(dtm)).

### In memory DTM {#engine-dtm-inmemory}
...
## Height normalization {#engine-normalization}

Previous sections detail considerations for using the `LAScatalog` engine to perform point cloud normalization using `normalize_height()` (see section \@ref(norm)). We created a DTM in the previous section, so we use it here to normalize.
...
## Individual Tree Detection {#engine-itd}
...
## Individual Tree Segmentation {#engine-its}

Individual tree segmentation with `segment_trees()` is the most complicated function to use with a `LAScatalog` because there are many different algorithms. Some imply the use of an in memory raster and an in memory vector, some require only an in memory raster and some require only a point cloud (see section \@ref(itd-its)). Moreover it is complex to manage edge artifacts. Imagine there is a tree that is situated exactly on the edge between two files. The first half on one file, the second half on another. The two files will be processed independently - maybe on different remote computers. Both sides of the tree must be attributed the same ID independently to get something that is strictly continuous. The function `segment_trees()` manages all these points. The example below uses the `dalponte2016()` algorithm.
...
## Retile a catalog {#engine-retile}
...
## The case of ground inventories {#engine-independent-files}
...
## Summary of available options {#engine-summary-sheet}
