...
The function `readLAS()` reads a LAS or LAZ files an return an object of class `LAS`. The `LAS` format class is documented in depth in a [dedicated vignette]() and the follwing chapter is likely to repeat some information given is the mentionned document. 
...
```r
las <-  readLAS("file.las", select = "xyzi", filter = "-keep_first -drop_z_below 5 -drop_z_above 50")
```
...
The `lidR` package take advantage of the `rgl` package to provide a versatile and interactive 3D viewer with point colored by default by their Z coordinates and displayed on a black background.
...
A check is performed at read time anyway. But the read time check does not go as deep as `las_check()` for computation time reasons.
