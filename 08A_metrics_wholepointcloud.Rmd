...
We have computed man metrics for each plot and the known value of interest. We can use that to build a linear model with some metrics.
...
In section \@ref(engine) we will study how to extract a ground inventory and in section \@ref(modeling) we will study more in depth modelling.
