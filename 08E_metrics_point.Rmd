...
The "point" level of regularization corresponds to the computation of derived metrics for each point of the point cloud using its neighborhood to define a local subset. Function `point_metrics()` allows to generate point-based derived metrics and works like `cloud_metrics()`, `grid_metrics()`, `hexbin_metrics()`, `tree_metrics()` or `voxel_metrics()` seen in chapters \@ref(metrics), \@ref(cba), \@ref(aba), \@ref(tba) and \@ref(vba) and the reader can refer to previous sections for a more comprehensive overview.

For each point the neibourhood can be either (1) the k nearest neighbors, (2) the points within a sphere centered on the processed point or (3) the k nearest neighbors but with a limited radius search. The syntax for the 3 options is given below.
...
The output is a `data.table` with the ID of the reference points and the metric(s) for this points.

```{r}
metrics
```

Intead of the ID on may prefer to get the coordinates with `xyz = TRUE`

```r
metrics <- point_metrics(las, ~list(imean = mean(Intensity)), k = 7, xyz = TRUE)
```

It is also possible to process only some points excluding the other one from the computation without creating a copy of the point cloud. In the following, the mean intensity is computed excluding the outliers with an intensity above 100. No metric is computed for those points and they are not used to get the local neighborhood of the other points.

```{r}
metrics <- point_metrics(las, ~list(imean = mean(Intensity)), k = 7, filter = ~Intensity < 100 )
metrics
```
...
Multispectral ALS data are sampled with 3 devices each emmiting a different wavelength. The point cloud is usually provided in the form of 3 `las` files, each file corresponding to a spectral wavelength. No matter the actual wavelenght we can consider the first band as blue, the second as red and the third one as green and thus consider that each point has a pure color.
...
An outlier is a point that belong alone compared to other points. Outliers are usually hight points with no close neighbors. The term "no close neighbor" can have several formal definition such as *no neighbors closer than x meters* or *less than n neighbors within a distance x meter* or any other fair definition. In all case it can be estimated with a metrics and thus `point_metrics()` enable to design an outlier filter method. We give this as an exercice to the reader.
