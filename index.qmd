...
-   April 2026: new section about COPC and remote files in @sec-copc
-   March 2026: significant update of the ground classification chapter with the introduction of the progressive TIN densification.
-   March 2026: update of the CHM chapter with the introduction of the spike-free algorithm.
-   March 2026: update of the CHM chapter with post-processing methods.
...
For a package focused on production rather than research and development, consider the [`lasR`](https://github.com/r-lidar/lasR) package, which is not covered in this book. The `lasR` package is much more powerful than `lidR`, can be used in `python` and is designed to process terabytes of data. However, the trade-off is that it does not allow for easy and convenient manipulation of point clouds in R like `lidR`. Both tools are intended for different use cases.

# Installation

::: {.callout-warning}
Since august 2026, the `lidR` package is no longer on CRAN.
:::
