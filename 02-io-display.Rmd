...
The `trim` parameter enables trimming of values when outliers break the colour palette range. For example, intensity often contains large outliers. The palette range would be too large and most of the values will be considered as "very low", so everything will appear in the same colour. Using `trim` as an outlier threshold enable to print every value higher than `trim` using the same colour.
