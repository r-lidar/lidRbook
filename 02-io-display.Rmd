...
It is important to understand that the option `filter` in `readLAS()` keeps or discards point **at read time** i.e. while reading at the C++ level independently of R. Some users have already reported misunderstanding of the behavior of this options because it is different from other `filter` functions users may have encountered in other packages and that work at the R level.

For exemple the R function to filter points of interest (POI) is `filter_poi()`. In the following example we are (1) reading only the first returns and (2) reading all the points then filtering the first return.
...
Both outputs are strictly identical but the first one is faster and more memory efficient. We will also see that it is the only way to load only some POI in some cases in the chapter about `LAScatalog`. - **are the last 2 paragraphs really needed in this? Seems overkill for this**
...
`lidR` provides some easy to use functions for common overlay. For example `add_dtm3d()` to add a digital terrain model (**ADD LINK TO SECTION** see dedicated section) and `add_treetops3d()` to visualize the output of an individual tree detection (**ADD LINK TO SECTION** see dedicated section)
...
It is also possible to combine two point clouds with different color palettes. In the following example were are using a previously classified point cloud. We first separate the vegetation and non vegetation points using `filter_poi()` and then plot both on top of each other with different color schemes using `add` options in `plot()`
