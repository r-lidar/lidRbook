...
* `kriging` - Interpolation is done using a k-nearest neighbour (KNN) approach with an inverse distance weighting (IDW). This is a fast but basic method for spatial data interpolation.
...
![](images/norm/plot_terrainlaz.png)
...
```{r}
#Use lasfilter to keep only points classified as ground (class 2)
las_grnd <- lasfilter(las,Classification==2)
```
...
![](images/norm/plot_terrainground.png)
...
```{r,eval=F}
plot(las_grnd,color="Classification")
```

<center>

![](images/norm/plot_terrainclass.png)
...
```{r,eval=F}
#Normalize Terrain.laz using the 1 m DTM raster
