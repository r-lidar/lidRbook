...
The goal of this section is to describe the notion of _"metrics"_ in `lidR`. Analyses of point cloud data are often based on metrics calculations. Metrics are scalar summaries of point distributions that can be computed using varying neighborhood sizes for reference locations. For example, when reference locations are regularly distributed on a grid and the neigbourhood is a square centered on those locations, we typically use an area-based-approach (ABA) that consists of computing metrics for pixels. Maximum height of points inside a 20 x 20 m grid cell (in this case our pixel) is an example of an ABA metric. This however is not the only option, and other definitions can enable other analyses and applications.

The standard deviation of point heights within a single tree crown is an example of a metric calculated at the tree level. The average distance between a point and its k-nearest neighbours is a metrics calculated at the point level. More complex metrics can even be imagined like the average distance between first and last returns within a pixel or within a tree. In the end, irregardless of the scale at which metrics are calculated, they serve as proxies of forest inventory attributes or can be used as independent variables in predictive models.
...
## The basics
While there are some differences between functions dedicated to metric computations but the basic idea is the same for all of them.

In addition to the input point cloud, the user needs to provide a formula to calculate the metric(s) of interest. For example, the average height of points for each point cloud, each pixel, each tree crown, or each voxel can be be calculated using `mean(Z)`. 
...
In the example above only a single metric is calculated - the mean elevation of the points. Calculations can however be easily extended to any number of metrics. To do this, users can design custom functions. 

The function can contain any number of metrics, but needs to return a labeled `list`. For example, to calculate the mean and standard deviation of elevation & the mean intensity, the following function can be used:
...
In each case, outputs will be a different class (i.e. a `list`, a `RasterBrick`, a `SpatialPointsDataFrame` or a `data.frame`) to take advantage of the best storage method, but the same two metrics will be calculated for every unit of analysis. 
