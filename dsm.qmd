...
More advanced algorithms have also been designed that avoid pits during the computation step instead of requiring post-processing. [Khosravipour et al. (2014)](https://www.sciencedirect.com/science/article/abs/pii/S0303243416300873) proposed a ‘pit-free’ algorithm, which consists of a series of sequential height thresholds where Delaunay triangulations are applied to first returns. For each threshold, the triangulation is cleaned of triangles that are too large, similar to the example given in the previous section. In a final step, the partial rasters are stacked and only the highest pixels of each raster are retained (figure below). The output is a DSM that is expected to be natively free of pits without using any post-processing or correction methods.
...
## Spike-free algorithm {#sec-spikefree}

Another more advanced algorithm has also been designed that avoids pits during the computation step instead of requiring post-processing. [Khosravipour et al. (2016)](https://www.sciencedirect.com/science/article/abs/pii/S0303243416300873) proposed a “spike-free” algorithm, which consists of an incremental Delaunay triangulation of all returns with constraints on the subdivision of the triangles to locally “freeze” the triangular irregular network (TIN).

The algorithm has two parameters. `freeze_distance` should be set to **three times** the point spacing (or slightly more than three). The `buffer_distance` parameter must be kept as is.



```{r plot-chm-spikefree, fig.height=5.9, fig.width=6.9}
chm <- rasterize_canopy(las, res = 0.5, spikefree(1.5))
plot(chm, col = col)
```

## Post-processing a CHM {#sec-chm-post-process}

CHMs are usually post-processed to smooth or to fill empty pixels. This can be avoided with the `Pitfree` and the `Spikefree` but it is sometime mandatory with other methods.

### Using `terra`

One may want to apply a post-processing step to their CHM. The `terra` package allows simple raster processing. The code below presents a simple option to fill NAs and smooth a CHM with the `terra` package. For more details see the documentation of the package `terra`.
...
### Using St-Onge 2008 method

Another option is to use the CHM oriented raster enhancement algorithm by [St-Onge (2008)](https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=81365288221f3ac34b51a82e2cfed8d58defb10e).

```r
chm = pitfill_stonge2008(chm)
```
