...
Now we can compute the average in an ABA (see chapter \@ref(aba)).

```{r plot-raster-avg-dreturns, fig.width=5.4, fig.height=5.2}
d_first_last <- grid_metrics(las, ~mean(d), 15)
plot(d_first_last, col = height.colors(50))
```

Now we have a working method, we can extend the idea to apply it over a broad coverage using the `LAScatalog` processing engine (chapter \@ref(engine)). First we create a function that does this job. This is not mandatory but factorizing the code is good practice.
...
Then we define a function that respects some requirement imposed by the `LAScatalog` processing engine (chapter \@ref(engine2)).
...
```{r plot-raster-multispectral-rgb}
rgb <- grid_metrics(las, ~colorize(Intensity, ScannerChannel), 2)
plotRGB(rgb, axes = T)
```

We can also attribute an RGB color per voxel, discarding the voxels where `RGB = NA` by using `colorize()` with `voxel_metrics()` (see chapter \@ref(vba)).
...
To finish we can `colorize()` with `point_metrics()` (see chapter \@ref(pba)). This has the advantage of maintaining much more points compared to the voxel-based version and to preserve coordinates.
