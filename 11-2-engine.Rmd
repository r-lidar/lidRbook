...
ctg_norm <- normalize_elevation(ctg, dtm) #normalize
...
This method is however far from optimal. First, **the whole** point cloud is read 3 times. First time to create the DTM, second to normalize, and  third to colorize. This takes a significant amount of time. It also implies the need to create copies of the acqusition after each processing step, taking up a lot of disk storage. Then, to finish, it a custom `for` loop is used because `merge_spatial()` cannot be applied on a `LAScatalog`. 

<c>
**What if we were able to do that in a single run?**
</c>

In this chapter we outline the `catalog_apply()` function, which is used subliminally every function seen in the previous chapter. We show how to leverage its versatility to design tools that do not yet exist using taking the DTM + normalization + colorization example from above. 

Again, the `LAScatalog` class and the `LAScatalog` engine are deeply documented in a two dedicated vignettes available [here](https://cran.r-project.org/web/packages/lidR/vignettes/lidR-LAScatalog-class.html) and [here](). The purpose of these examples is to provide alternative, hands on documentation with more applied examples and real use cases. 

In this chapter we will use the same 9 files used in chapter \@ref(engine1) and an RGB raster that encompasses the collection of file.
...
The satellite images are available for download [here](data/ENGINE/catalog/rgb.tif).
...
First we need to create a function that performs the DTM + normalization + colorization in one call. Let's call it `dtm_norm_color()`. It is simple and uses only 3 functions.
...
The core engine is the function `catalog_apply()` but this function does not work with any user-defined function. User-defined functions must respect a specific template (**maybe link to the documentation?**) and is expected to perform some specific tasks. Firstly, the primary variable must be a chunk from the catalog within the function (see `chunk` below), the function must read the chunk, check it is not empty, then perform the computation.
...
This introduces an additional level of complexity that is crucial. First, catalog chunks will be sequentially fed into the function. Each chunks is read inside the user's function with all processing options being automatically respected (`filter`, `select`, `chunk_size`, `chunk_buffer` etc.). The `las` variable in the code snippet above is a point cloud extracted from the catalog that contains the chunks + a buffer. It's important to check that the loaded portion of the collection is not empty -- subsequent code will fail. This may happen depending on the size of the chunk and the `filter` options chosen. 

This function is `catalog_apply()` compatible and can applied over the entire lidar acquisition. The output will be a point cloud so we need to pay attention mitigate memory issues and save to disk. At this stage, we havn't mitigated that problem yet so we add it below.
...
output <- catalog_apply(ctg, dtm_norm_color_chunk, rgbmap = rgbmap) #impliment user-defined function using catalog_apply
output
```
...
We can use the option `automerge` so `catalog_apply()` will automatically merge the list into something more user-friendly.
...
We still have a problem here. The files are overlapping because we read each file with a buffer and then the outputs have been written **with their buffer**. This is bad practice. It is important to always remove the buffer after the computation. In this specific case the output is a LAS file. When `readLAS()` reads a chunk from the catalog the points in the buffer are flagged so they can be easily manipulated. Since they are flagged, we can remove these points easily by adding this to our user-defined function. It is always the role of the user to remove the buffer of the output.
...
Nice! We created a custom function -- `dtm_norm_color()` -- and we upscale its definition to capably and efficciently process an entire lidar acquisition made up of hundreds of files. Using options described in \@ref(engine), we can also introduce paralellization, chunk size control, and buffer size control.
...
We can check that it worked by loading a sample from somewhere in our new catalog. We are expecting to get a normalized (we normalized), colored (we merged satellite image), ground points (we used `-keep_class 2`). ---- **the user doesn't see that we did this from line 120**
...
When designing tools that will be used by third parties, we would like to hide `catalog_apply()` and intermediate functions inside a more user-friendly function similarly to how `lidR` functions that work the same both with a `LAS` or a `LAScatalog`. Moreover we would like to make the function foolproof for the users. To do this we can create 3 functions `dtm_norm_color()` with a [S3 dispatch](http://adv-r.had.co.nz/S3.html) as a function of the input. 
