...
--- JR do we have something for this section? --
...
Using the `data.table` [syntax](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) we can compute the distance between first and last returns for each pulse. It could be done with `dplyr` as well using `group_by() %>%  mutate()` but Jean-Romain is proponent of using `data.table()`.
...
Now we have a working method, we can extend the idea to apply it over a broad coverage using the `LAScatalog` processing engine -- see section \@ref(engine). First we create a function that does this job. This is not mandatory but factorizing the code is good practice.
...
We can use `colorize()` with `grid_metrics()` in an ABA. It returns a multi-layer raster that can be interpreted as RGB
...
Here we attribute an RGB color per voxel, discarding the voxels where `RGB = NA` by using `colorize()` with `voxel_metrics()` in a VBA (see section \@ref(vba)).
...
To finish we can `colorize()` with `point_metrics()` in a PBA (see section \@ref(pba)). This has the advantage of maintaining much more points compared to the VBA version and to preserve coordinates.
