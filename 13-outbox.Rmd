...
We have presented many common processes through these tutorials. Most examples presented could have been done with other softwares, but what we have seen has only the tip of the iceberg. In fact, `lidR` was never designed with common processes in mind, but instead to explore new ones. Examples of this can be found in chapters like \@ref(Point Based Approach). 

The following chapter presents some applications that are likely to be impossible to reproduce in other softwares, demonstrating how we can go from a novel idea to a fully functional tool using `lidR`. 

## New complex metrics in ABA

### Distance between returns {#outboxdistancereturn}

In forestry simple metrics can be derived from first returns. But what about metrics that estimate the average distance between first and last returns? Is is a valuable metrics for characterizing of forest stucture? Can we refine the prediction? Nobody knows, but we can try. 
...
Using the `data.table` style we can compute the distance between first and last returns for each pulse. It could be done with `dplyr` as well using `group_by() %>%  mutate()` but Jean-Romain is proponent of using `data.table()`.

```r
las@data[, d := Z[1]-Z[2], by = pulseID][]
```
**Might want to add some comments in these for people who are totally fresh to the syntax**
```{r, echo = FALSE}
u = las@data[, .(d = Z[1]-Z[2]), by = pulseID]
las@data = las@data[u, on = "pulseID"]
```

At this stage our data set contains distances twice. One associated with the first return and the second with the last return. We only need a single value so lets keep only first returns. Some distances are `NA` and correspond to pulses at the very edge of the file. We can discard those case for the moment.
...
Now we have a working method, we can extend the idea to apply it over a broad coverage using the `LAScatalog` processing engine -- see \@ref(lascatalog). First we create a function that does this job. This is not mandatory but factorizing the code is good practice.
...
To finish we use the `LAScatalog` processing engine with `cataog_sapply()`. We need to process with a buffer to avoid the case where pulses are separated in two files, so we use `opt_chunk_buffer(ctg) <- 2`. In addition, with the output being a raster, we ensure alignment of the chunk with the raster.
...
We have already seen a multispectral coloring example in section \@ref(sec:pba-mscoloring). This is truly a good example of how to think out of the box with `lidR` so we show several variants here. The goal is to use multispectral data to generate false coloring and see how false coloring may be used for further analyses.

Multispectral ALS data are sampled with 3 devices each emmiting a different wavelength. The point cloud is usually provided in the form of 3 `las` files, each file corresponding to a spectral wavelength. No matter the actual wavelength we can consider the first band as blue, the second as red and the third one as green and thus consider that each point has a pure color.
...
Each channels returns vary in intensities as seen in thefigure below because of the wavelength reflectance properties of the targets. A first step could be to normalize the intensities. For the need of this example we won't do that to keep the example simple. 
...
Also in this very specific example we can notice that only few intensities are above 255, So intensity ranges from 0 to almost 255. This is very convenient because we will be able to use the raw values as color without transformation. We will simply force values above 255 to be 255. This is a dirty option but fair enough for the example which is not about intensity normalization
...
Now we can use this function with different levels of regularization
...
Here we attribute nn RGB color per voxel, discarding the voxels with RGB = NA by using `colorize()` with `voxel_metrics()` in a \@ref(VBA).
...
To finish we can compute `colorize()` with `point_metrics()` in a \@ref(PBA). This has the advantage to maintain much more points compared to the VBA version and to preserve the coordinates.
