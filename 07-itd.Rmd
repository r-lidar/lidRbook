...
```{r plot-chm-tree-itd, fig.height=6.9, fig.width=7}
...
The number of detected trees is correlated to the `ws` argument. Small windows sizes usually gives more trees, while large windows size generally miss smaller trees that are _"hidden"_ by big trees that contain the highest points in the neighborhood. This can be seen in the figure below where too many trees are found with a small window size and only the dominant trees are found with a large windows size.

```{r plot-chm-tree-ws, fig.height=5.4, fig.width=10}
ttops_3m <- find_trees(las, lmf(ws = 3))
ttops_11m <- find_trees(las, lmf(ws = 11))

par(mfrow=c(1,2))
plot(chm, col = height.colors(50))
plot(ttops_3m, add = TRUE)
plot(chm, col = height.colors(50))
...
```{r plot-chm-tree-vws, fig.height=5, fig.width=5.2}
...
There is no intrinsic limitation to the user-defined function. One does however need to pay attention to special cases. For example, if a point is below 0 m the function above may compute a negative windows size and the detection will fail. Similarly, if an outlier is encountered a massive window may be computed and lead to suspicious results. We therfore recommend using a more robust function with some built in thresholds.
...
```{r plot-multichm-lmf, fig.height=15, fig.width=9.8}
par(mfrow=c(3,2))
...
It's also possible to compute metrics for each tree. We go further into this functionality in section \@ref(tba), but here is a brief example where crowns are colored by their area.

```{r plot-tree-hulls-metrics-its, fig.height=4, fig.width=4, warning=FALSE}
...
```{r plot-raster-its, fig.height=5, fig.width=5.2}
algo <- dalponte2016(chm_p2r_05_smoothed, ttops_chm_p2r_05_smoothed)
crowns <- algo()
...
By comparing their crowns with `delineate_crowns()` we can more easily see that the `li2012` algorithm doesn't perform well in this example with default parameters - maybe some parameter tuning can lead to better results!


```{r plot-compare-hulls, fig.height=5, fig.width=5.8, cache = TRUE}
