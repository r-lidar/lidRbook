...
The number of detected trees is correlated to the `ws` argument. Small windows sizes usually gives more trees, while large windows size generally miss smaller trees that are _"hidden"_ by big trees that contain the highest points in the neighborhood. This can be seen in the figure below where only the dominant trees are found with a large windows size and too many trees are found with a small windows size.
